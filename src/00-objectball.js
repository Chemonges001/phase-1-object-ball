...
function teamName (nameOfTeam){
  ...
  for( const gameKey in game){
    ...
    for(const teamKey in teamObj){
      if (teamKey === nameOfTeam){
        return teamObj[teamKey].teamName
      }
    }
  }
}
...
for(const gameKey in game){
  ...
  for(const playerKey in playerObj){
    if (playerKey = playerName){
      let stats = Object.values(teamObj.players)
      return stats
    }
  }
}
...
function bigShoeRebounds(playerName){
  ...
  for ( const gameKey in game){
    const teamObj = game[gameKey]
    const playerProfile = teamObj.players

    for (key in playerProfile){
      const bigShoeSize = Math.max(...shoeSize)
      if(playerName === bigShoeSize){
        return playerObj[playerName].rebounds
      }
    }
  }
}
